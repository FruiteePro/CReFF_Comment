...
class Global(object):
    ...
    # 服务器端学习图像特征
    def learn_feature(self, args, new_global_params, old_global_params):
        # 获取 feature_net 的当前参数字典
        new_feature_net_params = self.feature_net.state_dict()
        # 将新的全局模型的分类器拷贝进来
        for name_param in reversed(new_global_params):
            if name_param == 'classifier.bias':
                new_feature_net_params['bias'] = new_global_params[name_param]
            if name_param == 'classifier.weight':
                new_feature_net_params['weight'] = new_global_params[name_param]
                break
        # 将参数加载进模型
        self.feature_net.load_state_dict(new_feature_net_params)
        # 将分类器参数转换为列表
        new_net_global_parameters = list(self.feature_net.parameters())
        # 加载旧的全局模型的分类器
        feature_net_params = self.feature_net.state_dict()
        for name_param in reversed(old_global_params):
            if name_param == 'classifier.bias':
                feature_net_params['bias'] = old_global_params[name_param]
            if name_param == 'classifier.weight':
                feature_net_params['weight'] = old_global_params[name_param]
                break
        self.feature_net.load_state_dict(feature_net_params)
        # 设置为训练模式
        self.feature_net.train()
        # 创建优化器
        optimizer = SGD(self.feature_net.parameters(), lr=args.lr_net) 
        ...
        for i in range(args.num_epochs_local_training):
            ...
            for data_batch in trainloader_ft:
                ...
                images, labels = images.to(self.device), labels.to(self.device)
                # 将数据输入到模型中进行计算，得到输出
                output_syn = self.feature_net(images)
                # 计算输出和标签的损失值
        ...
        #  取出 feature_net 的分类器参数
        re_feature_net_params = list(self.feature_net.parameters())

        # # 使用均方误差作为损失函数
        # criterion = nn.MSELoss()  
        # criterion = nn.CrossEntropyLoss()
        # # 创建特征的优化器
        # optimizer_feature_syn = SGD([self.feature_syn, ], lr=args.lr_feature)  # optimizer_img for synthetic data

        # 将参数转化为张量
        new_net_global_parameters_tensor = torch.cat([param.view(-1) for param in new_net_global_parameters])
        # re_net_global_parameters_tensor = torch.cat([param.view(-1) for param in re_feature_net_params])

        # loss_feature = torch.tensor(0.0).to(args.device)
        # loss_feature += match_loss(new_net_global_parameters_tensor, re_net_global_parameters_tensor, args)
        ...
        for ep in range(20):
            ...
            re_net_global_parameters_tensor = torch.cat([param.view(-1) for param in re_feature_net_params])
            # 计算两组参数的均方误差
            # loss_feature = criterion(new_net_global_parameters_tensor, re_net_global_parameters_tensor)
            loss_feature += match_loss(new_net_global_parameters_tensor, re_net_global_parameters_tensor, args)
            # loss_feature = criterion(new_net_global_parameters_tensor, re_net_global_parameters_tensor)
            # 将优化器参数梯度清零
            self.optimizer_feature.zero_grad()
        ...
        # loss_feature = criterion(new_net_global_parameters_tensor, re_net_global_parameters_tensor)
        loss_feature += match_loss(new_net_global_parameters_tensor, re_net_global_parameters_tensor, args)
        return loss_feature.item()
    
    # 重新训练分类器
    def classifer_re_train_feature(self, epochs_re_training, batch_size_local_training):
        # 取出模型的分类器参数
        classifier_params = self.syn_model.classifier.state_dict()

        # 将参数加载进分类器网络
        self.feature_net.load_state_dict(classifier_params)
        # 设置为训练模式
        self.feature_net.train()
        ...
        # 创建优化器
        optimizer = SGD(self.feature_net.parameters(), lr=args.lr_feature) 
        ...
        # 将分类器加载回模型
        self.syn_model.classifier.load_state_dict(self.feature_net.state_dict())
...
def Fedavg_Re_train_1():
    ...
    for r in tqdm(range(1, args.num_rounds+1), desc='server-training'):
        ...
        # 服务器学习图像特征
        if r > 50 :
            loss_feature = global_model.learn_feature(args, copy.deepcopy(fedavg_params), copy.deepcopy(global_params))
            feature_loss.append(loss_feature)
        # global eval
        # 全局模型推理，计算准确度
        one_re_train_acc = global_model.global_eval(fedavg_params, data_global_test, args.batch_size_test)
        # 将本轮准确率存储
    ...
    # 保存训练模型参数
    # torch.save(global_model.syn_model.state_dict(), 'global_model_fedavg_200_epochs.pth')
    torch.save(global_model.feature_syn, 'feature_syn.pth')
    ...
    for i in tqdm(range(20), desc='classifer-re-training'):
        # 重新训练分类器
        global_model.classifer_re_train_feature(10, args.batch_size_local_training)
        # global eval
        one_re_train_acc = global_model.global_eval_no_paramsload(data_global_test, args.batch_size_test)
        # 将本轮准确率存储
        re_train_acc.append(one_re_train_acc)
    ...
    # 保存模型
    # torch.save(global_model.syn_model.state_dict(), 'global_model_fedavg_re_train_20_epochs.pth')
...
if __name__ == '__main__':
    ...
    if args.model == 'CReFF':
        print("CReFF Running")
        CReFF()
    elif args.model == 'Fedavg_Re_train':
        print("Fedavg_Re_train Running")
        Fedavg_Re_train()
